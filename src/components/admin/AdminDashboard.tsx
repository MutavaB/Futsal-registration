        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleLogout}
              className="flex items-center gap-2 text-sm font-semibold text-gray-300 hover:text-white bg-white/10 hover:bg-futsal-red px-4 py-2 rounded-lg transition-colors">
        ...
        <div className="flex gap-6">
                        ...
                        <tr key={p.id} onClick={() => setSelected(p)}
                          ...
                          <td className="px-4 py-3">
                            <button onClick={(e) => { e.stopPropagation(); handleDelete(p.id); }}
                              className="text-red-400 hover:text-red-600 p-1.5 rounded hover:bg-red-50 transition-colors" title="Delete">
          ...
          {selected && (
            <div className="w-80 flex-shrink-0 hidden xl:block">
              <div className="card border-t-4 border-futsal-red sticky top-24 overflow-y-auto max-h-[calc(100vh-7rem)]">
                <div className="p-5 border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-14 h-14 rounded-full bg-futsal-navy text-white font-black text-xl flex items-center justify-center border-2 border-futsal-red flex-shrink-0">
                ...
                <div className="p-5 space-y-3 text-sm">
                      ...
                      {(["Active", "Pending", "Inactive"] as Player["status"][]).map((s) => (
                        <button key={s} onClick={() => handleStatusChange(selected.id, s)}
                          className={`flex-1 flex items-center justify-center gap-1 text-xs font-bold py-1.5 rounded-lg border transition-all ${
                            selected.status === s
                              ? statusConfig[s].color + " scale-105 shadow-sm"
                  ...
                  <button onClick={() => handleDelete(selected.id)}
                    className="w-full mt-1 flex items-center justify-center gap-2 text-sm font-semibold text-red-500 hover:text-white hover:bg-red-500 border border-red-200 hover:border-red-500 py-2 rounded-lg transition-all">
